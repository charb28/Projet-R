...
CREATE INDEX index1 ON Salle (No_salle);
...
CREATE TABLE Seance (
   ...
   foreign key (Nom_cinema) REFERENCES cinema(Nom_cinema),
   foreign key (No_salle) REFERENCES Salle(No_salle),
   foreign key (ID_film) REFERENCES FILM(ID_film)
);
...
SELECT Nom_cinema FROM cinema WHERE Arrondissement = '12ème';

SELECT Nom, Prenom FROM artiste WHERE Nom LIKE 'H%';

SELECT nom, prenom FROM artiste  WHERE Annee_naissance IS NULL;

SELECT COUNT(*) AS NombreDeFois
FROM Role
WHERE Nom_acteur = 'Willis' AND Nom_Role = 'McLane';

-- Exo2
SELECT nom, prenom FROM artiste join role on role.Nom_acteur = Artiste.Nom
WHERE Nom_Role = 'Tarzan';

SELECT nom, prenom FROM Artiste JOIN Role ON Artiste.Nom = Role.Nom_acteur JOIN Film ON Role.ID_film = Film.ID_film
WHERE Film.Titre = 'Vertigo';

SELECT Film.Titre, Seance.Heure_debut
FROM Film
JOIN Seance ON Film.ID_film = Seance.ID_film
JOIN Cinema ON Seance.Nom_cinema = Cinema.Nom_cinema
WHERE Cinema.Nom_cinema = 'Rex';

SELECT Film.Titre, Role.Nom_Role
FROM Film
JOIN Role ON Film.ID_film = Role.ID_film
JOIN Artiste ON Role.Nom_acteur = Artiste.Nom
WHERE Artiste.Nom = 'Allen' AND Artiste.Prenom = 'Woody';

SELECT A.Nom AS Nom_Metteur_en_scene, R.Nom_Role, F.Titre
FROM Artiste AS A
JOIN Role AS R ON A.Nom = R.Nom_acteur
JOIN Film AS F ON R.ID_film = F.ID_film
